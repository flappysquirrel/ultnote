...
export async function activate(context: ExtensionContext) {
  const controller = notebooks.createNotebookController('ultnote', 'ultnote', 'Ultnote');

  controller.supportedLanguages = ['rust', 'go', 'javascript', 'typescript'];
  ...
  context.subscriptions.push(commands.registerCommand('ultnote.kernel.restart', () => {
    window.showInformationMessage('Restarting kernel');
  }));
  context.subscriptions.push(commands.registerCommand('ultnote.search', searchNotes));
  context.subscriptions.push(commands.registerCommand('ultnote.openMain', openMain));
  ...
  context.subscriptions.push(workspace.registerNotebookSerializer('ultnote', new MarkdownProvider(), notebookSettings));
